...
public class GeoPath extends GeoObject implements Serializable {
     ...
     * point. The control point lies in forward direction from point 1 towards
     * point 2.
     * @param p0x point 0 x
     * @param p0y point 0 y
     * @param p1x point 1 x
     ...
     * @param p2x point 2 x
     * @param p2y point 2 y
     * @param controlPoint the control point coordinates are returned in this array
    ...
    public void smooth(double smoothness, double[][] points,
            int firstPointID, int nbrPoints) {

        if (smoothness <= 0. || NumericalUtils.numbersAreClose(0., smoothness)) {
            straightLines(points, firstPointID, nbrPoints);
            return;
        }

        final int lastPointID = firstPointID + nbrPoints - 1;
        ...
        // if the points[] array contains a point preceding firstPointID, use 
        // this point to construct a control point for the first line segment. 
        // Otherwise the first control point is identical to the start point.        
        int precedingPoint = Math.max(0, firstPointID - 1);
        double prevX = points[precedingPoint][0];
        double prevY = points[precedingPoint][1];

        float[] ctrlP1 = new float[2];
        float[] ctrlP2 = new float[2];

        // move to first point
        path.moveTo(points[firstPointID][0], points[firstPointID][1]);

        for (int i = firstPointID + 1; i < lastPointID; i++) {
        ...
        // The points[] array may contain points followin lasPointID. If so, use 
        // the next point to compute two control point points, otherwise only 
        // compute one control point.
        // The last control point in a line will be set to the end point.
        final double x0 = points[lastPointID - 1][0];
        final double y0 = points[lastPointID - 1][1];
        final double x1 = points[lastPointID][0];
        final double y1 = points[lastPointID][1];
        final int followingPoint = Math.min(lastPointID + 1, points.length - 1);
        final double x2 = points[followingPoint][0];
        final double y2 = points[followingPoint][1];
        bezierPoint(prevX, prevY, x0, y0, x1, y1, ctrlP1, smoothness);
        bezierPoint(x2, y2, x1, y1, x0, y0, ctrlP2, smoothness);
        path.curveTo(ctrlP1[0], ctrlP1[1], ctrlP2[0], ctrlP2[1], x1, y1);
    }
     ...
     * @param flatness The maximum distance between the smooth bezier curve and
     * the new straight lines approximating the bezier curve.
     * @param generalPath The GeneralPath that will receive the flattened path.
     * If null a new GeneralPath will be created.
     * @return Returns the passed generalPath if not null, or a new GeneralPath
     * otherwise.
     */
    ...
    /**
     * Converts all bezier lines to straight lines, and stores the resulting
     * path in an other GeoPath.
     *
     * @param flatness The maximum distance between the smooth bezier curve and
     * the new straight lines approximating the bezier curve.
     * @param geoPath The GeoPath that will receive the flattened path. If null
     * a new GeoPath will be created.
     * @return Returns the passed geoPath if not null, or a new GeoPath
     * otherwise.
     */
